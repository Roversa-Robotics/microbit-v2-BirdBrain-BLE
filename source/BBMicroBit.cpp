...
bool flashOn;  // Allows us to cancel flashing a message if we're asked to print a symbol before
               // we're done
...
{
    if ((displayCommands[1] & SYMBOL) &&
        commandLength >= 6) // In this case we are going to display a symbol
    {
    ...
    else if (displayCommands[1] & SCROLL) // In this state we print a message
    {
        ...
        messageLength =
            (displayCommands[1] & 0x1F); // This gets us the length of the message to print
...
void decodeAndSetPins(uint8_t displayCommands[])
{
    ...
    if ((displayCommands[4] & 0x30) == 0x20)
    {
        ...
        if (period > 0 && duration > 10)
        {
            ...
            newBuzz = true; // Turning this on to update the loop in case the event is already
                            // active
    ...
    {
        // Tells the sensor data function to treat this pin as an analog input and configures it as
        // an analog input
        pinsInputs[0] = true;
...
// Get and convert the accelerometer values to 8 bit format, and check if the shake bit should be
// set
...
// Get and convert the magnetometer values to an 8 bit Finch format - this function is probably
// wrong right now
